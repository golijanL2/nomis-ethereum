...
namespace Nomis.Utils.Contracts.Calculators
{
        ...
        where TTransactionIntervalData : class, ITransactionIntervalData, new()
    {
        ...
        /// </summary>
        public Func<TWalletStats, TWalletStats> StatsFiller()
        {
        ...
        public TWalletStats ApplyCalculators()
        {
            ...
            foreach (var calculator in calculators)
            {
                ...
                var statsFiller = methodInfo.Invoke(this, Array.Empty<object>()) as Func<TWalletStats, TWalletStats>;
                if (statsFiller == null)
                {
                    continue;
                }

                result = statsFiller.Invoke(result);
                if (result.NoData)
                {
                    return new()
                    {
